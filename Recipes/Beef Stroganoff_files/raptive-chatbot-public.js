(function() {
    ...
    const AB_FLAG_KEY = "creatorbot_adthrive_AB_enabled";
    const AB_FLAG_VAL_ON = "on";
    const AB_FLAG_VAL_OFF = "off";
    ...
    function shouldShowChatbotOnPage(config){
        let flag = AB_FLAG_VAL_OFF;

        if(params.has('raptive_debug')) {
            flag = AB_FLAG_VAL_ON;
        } else {
            let valueInSessionStorage = window.sessionStorage && window.sessionStorage.getItem(AB_FLAG_KEY);
            if(valueInSessionStorage) {
                flag = valueInSessionStorage;
            } else {
                if(config && config.trafficPercent && Math.random() < parseFloat(config.trafficPercent)) {
                    flag = AB_FLAG_VAL_ON;
                } else {
                    flag = AB_FLAG_VAL_OFF;
                }
    
                if (window.sessionStorage) {
                    window.sessionStorage.setItem(AB_FLAG_KEY, flag);
                }
            }
        }

        return flag === AB_FLAG_VAL_ON;
    }
    ...
    function insertChatButtonDOM(){
        ...
        let buttonDom = `<div class="raptive-chat-button">Talk to our AI Chatbot</div>
