...
const toggleMenuItemTextFave=function(event){if(!event.target.closest('.favorite-toggle-text'))return;event.preventDefault();if(cwp.favorites==='slickstream'){document.dispatchEvent(new CustomEvent('slick-show-discovery',{detail:{page:'favorites'}}))}}
...
document.addEventListener('click',function(event){toggleMenu(event);toggleSubMenu(event);toggleSearch(event);closeSearch(event);toggleFave(event);toggleMenuItemTextFave(event);faqToggle(event)});function initGrowMeSdk(){if(!window.growMe){window.growMe=function(e){window.growMe._.push(e)}
window.growMe._=[]}}
